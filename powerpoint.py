...
def generate_m(key, width):
    ...
    left_vertical = distribute_points_around_line(
        l,
        jnp.array([1, 1]), jnp.array([1, 9]), 1, width
    )
    middle_to_top = distribute_points_around_line(
        m,
        jnp.array([1, 9]), jnp.array([2.5, 5]), 1, width
    )
    middle_to_bottom = distribute_points_around_line(
        m_,
        jnp.array([2.5, 5]), jnp.array([4, 9]), 1, width
    )
    right_vertical = distribute_points_around_line(
        r,
        jnp.array([4, 9]), jnp.array([4, 1]), 1, width
    )
    full_letter = jnp.stack((left_vertical, middle_to_top, middle_to_bottom, right_vertical), axis=0)
...
def generate_a(key, width):
    ...
    left_slant = distribute_points_around_line(key,
        jnp.array([4.5, 1]), jnp.array([5.5, 9]), 1, width
    )
    right_slant = distribute_points_around_line(key,
        jnp.array([5.5, 9]), jnp.array([6.5, 1]), 1, width
    )
    cross_bar = distribute_points_around_line(key,
        jnp.array([4.75, 5]), jnp.array([6.25, 5]), 1, width
    )
...
def generate_r(key, width):
    ...
    vertical_line = distribute_points_around_line(key,
        jnp.array([7, 1]), jnp.array([7, 9]), 1, width
    )
    semi_circle = distribute_points_around_circle(key,
        jnp.array([7, 7.5]), 1.5, -90, 90, 1, width
    )
    leg = distribute_points_around_line(key,
        jnp.array([7, 6]), jnp.array([8.5, 1]), 1, width
    )
...
def generate_c(key, width):
    return (
        distribute_points_around_circle(key,
            jnp.array([11.5, 5]), 2, 90, 270, 1, width
        )
        * jnp.array([1, 2])[..., None]
        - jnp.array([0, 5])[..., None]
    )
...
def generate_o(key, width):
    return (
        distribute_points_around_circle(key,jnp.array([14, 5]), 2, 0, 360, 1, width)
        * jnp.array([1, 2])[..., None]
        - jnp.array([0, 5])[..., None]
    )
...
def generate_n(key, width):
    ...
    return jnp.stack(
        [
            distribute_points_around_line(a,
                jnp.array([5, 9]), jnp.array([5, 1]), 1, width
            ),
            distribute_points_around_line(b,
                jnp.array([5, 9]), jnp.array([7, 1]), 1, width
            ),
            distribute_points_around_line(c,
                jnp.array([7, 1]), jnp.array([7, 9]), 1, width
            ),
        ], axis=0
    )[idx]
...
def generate_i(key, width):
    return distribute_points_around_line(key,
        jnp.array([8.5, 1]), jnp.array([8.5, 9]), 1, width
    )
...
def distribute_points_around_line(key, start, end, num_points, width):
    ...
    x_values = base_x + jax.random.uniform(key_x2, shape=(num_points,))*width - width/2
    y_values = base_y + jax.random.uniform(key_y2, shape=(num_points,))*width - width/2
...
def distribute_points_around_circle(
   key, center, radius, start_angle, end_angle, num_points, width
    ...
    angles = base * (jnp.radians(end_angle) - jnp.radians(start_angle)) + jnp.radians(start_angle)
